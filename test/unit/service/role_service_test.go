...
func newTestRoleService(t *testing.T) (service.RoleService, *appmock.MockRoleRepository, *appmock.MockCache) {
	t.Helper()
	mockRepo := new(appmock.MockRoleRepository)
	mockCache := new(appmock.MockCache)
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	svc := service.NewRoleService(mockRepo, mockCache, logger)
...
func TestRoleService_Create_Success(t *testing.T) {
	...
	mockRepo.On("Create", ctx, mock.AnythingOfType("*model.Role")).
		...
		Return(nil)
	mockCache.On("DeleteByPrefix", ctx, "roles:list:").Return(nil)
